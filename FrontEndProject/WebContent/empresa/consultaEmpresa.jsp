...
<%
	EmpresaController app = new EmpresaController();
	List<Empresa> lista = app.listar(paginaAtual,limitePorPagina);
%>
...
<div class="container">
	<div class="row">
		...
		<div class="col-md-6 text-right">
			<a href="<%=request.getContextPath()%>/empresa/cadastroEmpresa.jsp"
		...
		<div class="col-md-12">
			<table class="table hoverable striped responsive-table">
				...
				<tbody>
					<%
					if(lista != null)
						for(Empresa dado : lista) {
					%>
					<tr>
						<td><%= dado.getNomeFantasia() %></td>
						<td><%= dado.getCidade()%></td>
						<td><%= dado.getRazaoSocial()%></td>
						<td class="mw-200">
						<a
							href="<%=request.getContextPath()%>/empresa/cadastroEmpresa.jsp?id=<%=dado.getId() %>"
							...
							<a							
							href="<%=request.getContextPath()%>/empresa/processarEmpresa.jsp?acao=remove&id=<%=dado.getId() %>"
		...
		<div class="col-md-12 text-center">
			...
			<button onclick="paginaAnterior('<%=request.getContextPath()%>/empresa/consultaEmpresa.jsp',<%=paginaAtual%>)" type="button"
				<%if (paginaAtual == 1)
				         out.print("disabled"); %>
				class="btn waves-effect waves-light indigo"><i class="mdi-av-fast-rewind"></i> Anterior</button>
			<button onclick="proximaPagina('<%=request.getContextPath()%>/empresa/consultaEmpresa.jsp',<%=paginaAtual%>)" type="button"
